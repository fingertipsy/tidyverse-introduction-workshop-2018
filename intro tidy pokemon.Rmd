...
Hadly Wickham: "Tidy datasets are easy to manipulate, model and visualize, and have a specific structure: each variable is a column, each observation is a row, and each type of observational unit is a table."

Let's take a look at some examples of tidy and untidy data: http://garrettgman.github.io/tidying/
This website is also a great resouce to recap and expand on what you will learn in this workshop. 

_______________________________________________________________________________________________________________

Now let's start our own analysis!
We will work with a cool pokemon dataset, which can be found here: https://www.kaggle.com/cadong/learning-r-s-dplyr-ggplot2-eda-with-pokemon/data
...
_______________________________________________________________________________________________________________ 30min
...
Because tibbles behave slightly differnt than dataframes, you might encounter a special function that expects a dataframe. You can always turn a tibble back into a dataframe by using as.data.frame(data)
...
Carefully looking through the dataset. What is the format? Is this tidy data? Discuss! (5min)
...
What happens when you switch the code lines back? How can you fix the problem? (5min)
...
First step: Look at the variables. There are a few variables that we might want to have as factor instead of character variables
...
Now it's your turn! Find more variables, that should be factors and transform them. Also try out what happens, when you don't specify the factor levels.
The str function allows you to look at the structure of your variables. You might be familiar with the Variable viewer in SPSS, which is similar. Look at the structure and the differences between integer variables and factor variables. (5min)

```{r}
# pokemon$type1 <- factor(pokemon$type1)
# pokemon$type2 <- factor(pokemon$type2)
# we will gather these later, so it doesn't make sense to factorize now
pokemon$legendary <- factor(pokemon$legendary)
str(pokemon)
```

Our data doesn't have a very nice long format. In the next step we will reshape our data into a better long format. First, take a look at our data set: Which variables should be gathered to make the data less wide and give us one observation/row? Take a piece of paper and draw how the final long data set should look like. Which variables will we have and which values do they take? Discuss this with your neighbour or as a group.
...
Take a look at how the code from above looks like, when we use the pipe. You understand the pipe operator when you can explain what the two print functions do and why we get again both datasets (pokemon and pokemon_long) although the print commands are empty. (10min)

```{r}
pokemon_long <- #this is our final result
...
No it's your turn: Try to gather all the stats for the pokemon into two variables: the type of stats (attack, defense...) and the value. 
Try to use the pipe operator!
If this is easy for you try to combine the previous code chunk and this one and make one single pipeline (10min)
...
We can now find out which pokemon type is the mightiest. To do so, we summarise our data set.
...
Look at the code chunk and try to understand the single transformations we pipe our data through. It will help you to understand what each line of code does when you look at how the result changes when you disable that step. You can do that by turning the line into a comment. A comment is text within your code, but it is ignored when your code is executed. You can turn text into comments by putting a # in front of it. (10min)
...
We now have a nice new summary dataset. It is a much better overwiev than the original dataset and allows us to easily compare pokemon types regarding their total strenght.
...
The code chunk below can do the trick. Try to fill in the blanks and make it work (10min)
...
We now want to do a violin plot, that compares the different statistics between legendary and non-legendary pokemon. Below you can see the code-chunk, that produces the plot. Try to fill in the blanks and make it work! (10min)
...
```
  ...
  This "introduction to the tidyverse" was created by Theresa Elise Wege in Feb 18 using the "pokemon with stats" dataset, shared by Alberto Barradas on kaggle.com
